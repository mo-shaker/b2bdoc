...
| **Partially Completed** | Some delivered, some remaining | A follow-up demand — **not created in Release 1** |
...
| In Execution → **Partially Completed** | Some but not all has been delivered. **Neither the transition nor the follow-up happens in Release 1** |
| In Execution → **Failed** | Nothing was delivered. **Neither the transition nor the follow-up happens in Release 1** |
| Partially Completed → **Completed** | A follow-up fulfils the remainder. **Unreachable in Release 1** — no follow-up is created |
