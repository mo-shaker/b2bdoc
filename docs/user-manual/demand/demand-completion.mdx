...
| 4 | The demand becomes **Partially Completed** | SCOP — **fails when a Warehouse user validates**, see below |
| 5 | A **follow-up demand** *should* be created for the remainder | SCOP — **never created**. The backorder carries the remainder instead |

:::danger[Verified defect — the demand does not advance when the **warehouse** validates]

Steps 4 and 5 are correct as designed, and step 4 works — **but not when a Warehouse user
presses Validate**, which is exactly who the manual tells to press it.

The Warehouse role has **read but not write** on `scop.demand`, so the projection that
advances the demand raises an access error. Generation is non-blocking, so the error is
swallowed into `Reporting → Failed Projections` and the transfer saves anyway:

```text
PickingDone   failed   You are not allowed to modify 'SCOP Demand' (scop.demand) records.
```

On `b2b` that is **13 failed projections against 13 processed** — half the log.

**The trap:** quantities still appear, because **Fulfilled** is computed from the Odoo moves.
Only the demand's **state** is stuck. Nothing on screen looks wrong.

**The logic itself is sound.** Run the same step as a Planner and the demand advances
immediately — `DMD-2026-000010` (6 of 10) to **Partially Completed**, `DMD-2026-000013`
(0 of 5) to **Failed**.

**Until it is fixed:** watch `Reporting → Failed Projections` daily and have an Administrator
press **Retry**. Retrying requires SCOP/Administrator; nobody below that can clear the queue.

Step 5 is separate: **no follow-up demand is ever created**. For a partial delivery that costs
nothing, because the Odoo **backorder** carries the remainder against the *same* demand —
verified end to end below. For a **total failure** there is no backorder, and nothing carries
the requirement.
...
| **In Execution** after its trip closed | The `PickingDone` projection failed — see `Reporting → Failed Projections` | An Administrator presses **Retry** |
