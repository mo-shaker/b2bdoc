...
<Screenshot
  ...
  alt="The SCOP Loading Tasks list showing two tasks for one trip, the first In Progress and the second still Pending."
  caption="Two shipments on one trip means two tasks. The second is still Pending, which is what holds the vehicle at the gate."
/>
...
<Screenshot
  ...
  alt="The SCOP Loading Tasks list showing both tasks Done with their started and completed times and actual durations against a 30 minute plan."
  caption="Both tasks settled on b2b. Actual duration is computed from the two timestamps — which is why Start has to be pressed when loading actually begins."
/>
