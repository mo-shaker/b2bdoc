...
| Test the platform yourself in 15 steps | [End-to-End Test Checklist](./end-to-end-test-checklist.mdx) |
...
:::warning[SCOP roles also need Odoo rights]

A SCOP role on its own is not enough. SCOP screens read Odoo records — transfers, vehicles, employees — so operational users also need the matching Odoo groups (**Inventory / User**, and **Fleet / Officer** for planners). Without them, screens such as the Demand form and the planning wizard raise an access error. This is recorded in [Release 1 Boundaries](./release-1-boundaries.mdx).
